...
describe Event do
  describe ".search" do
    ...
    let!(:event_1) { create(:event, :calendar => calendar1, 
      :start => 9.days.ago.to_i, :end => 1.day.ago.to_i) }
    let!(:event_2) { create(:event, :calendar => calendar1, 
      :start => 5.days.ago.to_i, :end => 4.day.ago.to_i )}
    let!(:event_3) { create(:event, :calendar => calendar2, 
      :start => 8.days.ago.to_i, :end => 7.day.ago.to_i )}
    let!(:event_4) { create(:event, :calendar => calendar3, 
      :start => 5.days.ago.to_i, :end => 1.day.ago.to_i)}

    let!(:event_5) { create(:event, :calendar => calendar4, 
      :start => 5.days.ago.to_i, :end => 1.day.ago.to_i)}

    let!(:event_6) { create(:event, :calendar => calendar4, 
      :start => 9.days.ago.to_i, :end => 1.day.ago.to_i)}
    ...
    context "when many users are involved" do
      ...
      it "ignores the data which doesn't belong to the current user" do
        ...
        params = Hashie::Mash[:calendar_ids => "#{calendar1.id},#{calendar2.id},#{calendar3.id},#{calendar4.id}"]
        Event.search(params, customer2).to_a.should == [ event_5, event_6 ]

        params = Hashie::Mash[:calendar_ids => calendar4.id.to_s]
